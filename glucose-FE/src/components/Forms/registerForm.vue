...
const validatePassword = () => {
    if (password.value.length < 6) {
        passwordError.value = '密碼至少需要6個字符';
        return false;
    }
    passwordError.value = '';
    return true;
...
const validateConfirmPassword = () => {
    if (password.value !== confirmPassword.value) {
        confirmPasswordError.value = '密碼不匹配';
        return false;
    }
...
<template>
    <div class="w-full max-w-md p-8 bg-white rounded-xl shadow-xl transition-all duration-300 hover:shadow-2xl transform hover:scale-102">
        <div class="flex justify-center mb-6">
            <div class="w-16 h-16 bg-gradient-to-r from-blue-500 to-teal-400 rounded-full flex items-center justify-center">
        ...
        <h2 class="text-center text-3xl font-extrabold text-gray-900 mb-2">建立您的帳戶</h2>
        <p class="text-center text-sm text-gray-600 mb-8">
            註冊以追蹤您的血糖健康數據並獲取AI個性化建議
        </p>
